...
LLM API 成本可能成为 AI Agent 规模化的瓶颈。通过缓存、模型选择、Token 压缩等手段控制成本。
...
# 快速验证
config = AgentConfig(name="成本优化")
agent = BasicCostOptimization(config)
result = agent.process("介绍 成本优化 的主要功能")
...
async def advanced_demo():
    config = AgentConfig(name="成本优化")
    agent = AdvancedCostOptimization(config)
...
class DistributedCostOptimization:
    """生产级 成本优化：支持分布式部署和高可用"""
    ...
    def __init__(
        ...
        self.primary = AdvancedCostOptimization(primary_config)
        self.fallback = AdvancedCostOptimization(fallback_config) if fallback_config else None
        self.cache: dict = {}
...
# 生产环境配置示例
primary = AgentConfig(name="成本优化", model="gpt-4o", temperature=0.0)
fallback = AgentConfig(name="成本优化-fallback", model="gpt-4o-mini", temperature=0.0)

distributed_agent = DistributedCostOptimization(
...
async def complete_exercise():
    ...
    # 2. 初始化服务
    service = AdvancedCostOptimization(config)
    print(f"2. 初始化服务成功")
...
public class CostOptimizationService {
    ...
    public CostOptimizationService(ChatClient chatClient, MeterRegistry meterRegistry) {
        this.chatClient = chatClient;
        this.meterRegistry = meterRegistry;
        this.requestCounter = Counter.builder("cost-optimization.requests")
            .register(meterRegistry);
        this.requestTimer = Timer.builder("cost-optimization.latency")
            .register(meterRegistry);
    }
    ...
    public ProcessResult process(String query, Map<String, Object> context) {
        ...
        try {
            ...
            log.error("成本优化 处理失败: {}", e.getMessage(), e);
...
// REST 控制器
@RestController
@RequestMapping("/api/v1/cost-optimization")
public class CostOptimizationController {

    private final CostOptimizationService service;
