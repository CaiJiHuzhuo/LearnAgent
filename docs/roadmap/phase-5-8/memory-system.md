...
Agent 的记忆系统决定了它能维持多长时间的对话连贯性，以及能积累多少用户和业务知识。
...
# 快速验证
config = AgentConfig(name="记忆系统")
agent = BasicMemorySystem(config)
result = agent.process("介绍 记忆系统 的主要功能")
...
async def advanced_demo():
    config = AgentConfig(name="记忆系统")
    agent = AdvancedMemorySystem(config)
...
class DistributedMemorySystem:
    ...
    def __init__(
        ...
        self.primary = AdvancedMemorySystem(primary_config)
        self.fallback = AdvancedMemorySystem(fallback_config) if fallback_config else None
        self.cache: dict = {}
...
# 生产环境配置示例
primary = AgentConfig(name="记忆系统", model="gpt-4o", temperature=0.0)
fallback = AgentConfig(name="记忆系统-fallback", model="gpt-4o-mini", temperature=0.0)

distributed_agent = DistributedMemorySystem(
...
async def complete_exercise():
    ...
    # 2. 初始化服务
    service = AdvancedMemorySystem(config)
    print(f"2. 初始化服务成功")
...
public class MemorySystemService {
    ...
    public MemorySystemService(ChatClient chatClient, MeterRegistry meterRegistry) {
        this.chatClient = chatClient;
        this.meterRegistry = meterRegistry;
        this.requestCounter = Counter.builder("memory-system.requests")
            .register(meterRegistry);
        this.requestTimer = Timer.builder("memory-system.latency")
            .register(meterRegistry);
    }
    ...
    public ProcessResult process(String query, Map<String, Object> context) {
        ...
        try {
            ...
            log.error("记忆系统 处理失败: {}", e.getMessage(), e);
...
// REST 控制器
@RestController
@RequestMapping("/api/v1/memory-system")
public class MemorySystemController {

    private final MemorySystemService service;
