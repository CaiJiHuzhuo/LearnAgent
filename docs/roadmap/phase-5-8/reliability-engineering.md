...
生产级 AI Agent 需要处理各种故障：LLM 超时、API 限流、工具失败。可靠性工程确保系统在故障下仍能运行。
...
# 基础 可靠性工程 实现
class BasicReliabilityEngineering:
    """基础 可靠性工程 实现"""
...
# 快速验证
config = AgentConfig(name="可靠性工程")
agent = BasicReliabilityEngineering(config)
result = agent.process("介绍 可靠性工程 的主要功能")
...
class AdvancedReliabilityEngineering:
    """进阶 可靠性工程 实现，支持异步、重试、监控"""
...
async def advanced_demo():
    config = AgentConfig(name="可靠性工程")
    agent = AdvancedReliabilityEngineering(config)
...
class DistributedReliabilityEngineering:
    """生产级 可靠性工程：支持分布式部署和高可用"""
    ...
    def __init__(
        ...
        self.primary = AdvancedReliabilityEngineering(primary_config)
        self.fallback = AdvancedReliabilityEngineering(fallback_config) if fallback_config else None
        self.cache: dict = {}
...
# 生产环境配置示例
primary = AgentConfig(name="可靠性工程", model="gpt-4o", temperature=0.0)
fallback = AgentConfig(name="可靠性工程-fallback", model="gpt-4o-mini", temperature=0.0)

distributed_agent = DistributedReliabilityEngineering(
...
async def complete_exercise():
    ...
    # 2. 初始化服务
    service = AdvancedReliabilityEngineering(config)
    print(f"2. 初始化服务成功")
...
public class ReliabilityEngineeringService {
    ...
    public ReliabilityEngineeringService(ChatClient chatClient, MeterRegistry meterRegistry) {
        this.chatClient = chatClient;
        this.meterRegistry = meterRegistry;
        this.requestCounter = Counter.builder("reliability-engineering.requests")
            .register(meterRegistry);
        this.requestTimer = Timer.builder("reliability-engineering.latency")
            .register(meterRegistry);
    }
    ...
    public ProcessResult process(String query, Map<String, Object> context) {
        ...
        try {
            ...
            log.error("可靠性工程 处理失败: {}", e.getMessage(), e);
...
// REST 控制器
@RestController
@RequestMapping("/api/v1/reliability-engineering")
public class ReliabilityEngineeringController {

    private final ReliabilityEngineeringService service;
