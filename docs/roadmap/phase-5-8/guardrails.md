...
护栏系统在 LLM 的输入和输出层添加安全检查，防止有害内容、越权操作和幻觉输出进入生产环境。

## 核心概念清单

- 护栏系统（Guardrails）的核心原理与应用场景
- 主流框架和工具对比
- 生产环境关键设计决策
- 与其他组件的集成方式
- 常见陷阱和解决方案
...
# 快速验证
config = AgentConfig(name="护栏系统（Guardrails）")
agent = BasicGuardrails(config)
result = agent.process("介绍 护栏系统（Guardrails） 的主要功能")
...
class AdvancedGuardrails:
    """进阶 护栏系统（Guardrails） 实现，支持异步、重试、监控"""
...
async def advanced_demo():
    config = AgentConfig(name="护栏系统（Guardrails）")
    agent = AdvancedGuardrails(config)
...
```python
# 护栏系统（Guardrails） 生产级实现：分布式、高可用
from typing import Optional
import uuid

class DistributedGuardrails:
    """生产级 护栏系统（Guardrails）：支持分布式部署和高可用"""
    ...
    def __init__(
        ...
        self.primary = AdvancedGuardrails(primary_config)
        self.fallback = AdvancedGuardrails(fallback_config) if fallback_config else None
        self.cache: dict = {}
...
# 生产环境配置示例
primary = AgentConfig(name="护栏系统（Guardrails）", model="gpt-4o", temperature=0.0)
fallback = AgentConfig(name="护栏系统（Guardrails）-fallback", model="gpt-4o-mini", temperature=0.0)

distributed_agent = DistributedGuardrails(
...
```python
# 护栏系统（Guardrails） 端到端练习
import asyncio

async def complete_exercise():
    """完整的 护栏系统（Guardrails） 练习流程"""
    print("=== 护栏系统（Guardrails） 练习 ===\n")

    # 1. 创建配置
    config = AgentConfig(
        name="护栏系统（Guardrails）",
        model="gpt-4o-mini",
        temperature=0.1,
        max_iterations=5
    )
    ...
    # 2. 初始化服务
    service = AdvancedGuardrails(config)
    print(f"2. 初始化服务成功")
...
```java
// 护栏系统（Guardrails） 的 Spring Boot 实现
@Service
@Slf4j
public class GuardrailsService {
    ...
    public GuardrailsService(ChatClient chatClient, MeterRegistry meterRegistry) {
        this.chatClient = chatClient;
        this.meterRegistry = meterRegistry;
        this.requestCounter = Counter.builder("guardrails.requests")
            .register(meterRegistry);
        this.requestTimer = Timer.builder("guardrails.latency")
            .register(meterRegistry);
    }
    ...
    public ProcessResult process(String query, Map<String, Object> context) {
        ...
        try {
            ...
            String response = chatClient.prompt()
                .system("你是专业的 护栏系统（Guardrails） 助手")
            ...
            log.error("护栏系统（Guardrails） 处理失败: {}", e.getMessage(), e);
