...
让 LLM 理解自然语言查询需求，自动生成并执行 SQL，为业务人员提供数据查询能力而无需学习 SQL。
...
# SQL 数据分析 Agent 的基础使用模式
class SqlDataAnalysisAgentConfig(BaseModel):
    """配置"""
    enabled: bool = True
...
class SqlDataAnalysisAgentClient:
    """基础客户端"""

    def __init__(self, config: SqlDataAnalysisAgentConfig):
        self.config = config
        self.client = OpenAI()

    def execute(self, task: str, context: Dict[str, Any] = None) -> Dict:
        """执行任务"""
        system_prompt = f"你是专业的 SQL 数据分析 Agent 执行器。按照用户要求完成任务。"
        user_msg = task
        if context:
...
# 基础使用示例
config = SqlDataAnalysisAgentConfig()
executor = SqlDataAnalysisAgentClient(config)
result = executor.execute("演示 SQL 数据分析 Agent 的基本用法")
...
class AdvancedSqlDataAnalysisAgent:
    """进阶 SQL 数据分析 Agent 实现"""
    ...
    def execute_step(self, step: Dict, previous_results: List) -> TaskResult:
        ...
        response = self.client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "你是 SQL 数据分析 Agent 执行专家，按步骤完成任务。"},
    ...
    def execute_plan(self, goal: str) -> TaskResult:
        """执行完整计划"""
        logger.info(f"开始执行 SQL 数据分析 Agent 任务: {goal[:50]}")
...
# 演示
agent = AdvancedSqlDataAnalysisAgent(SqlDataAnalysisAgentConfig())
result = agent.execute_plan("SQL 数据分析 Agent 在支付系统中的应用场景分析")
...
```python
# SQL 数据分析 Agent 生产级集成
import asyncio
from typing import AsyncGenerator
import uuid

class ProductionSqlDataAnalysisAgentPipeline:
    """生产级 SQL 数据分析 Agent 流水线"""
...
# 构建 SQL 数据分析 Agent 流水线
pipeline = ProductionSqlDataAnalysisAgentPipeline()
...
def stage_process(data: Dict) -> Dict:
    ...
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[{"role": "user", "content": f"处理 SQL 数据分析 Agent 请求: {data.get('query')}"}],
        max_tokens=200
    )
...
async def run_pipeline_demo():
    print(f"=== SQL 数据分析 Agent 流水线演示 ===")
    result = await pipeline.run({"query": "支付系统中 SQL 数据分析 Agent 的实际应用"})
    print(f"\n最终结果: {result['success']}")
    if result["success"]:
...
async def complete_optional_exercise():
    """完整的 SQL 数据分析 Agent 练习"""
    print(f"\n=== SQL 数据分析 Agent 可选专题练习 ===\n")

    # 初始化
    config = SqlDataAnalysisAgentConfig(debug_mode=True)
    client_basic = SqlDataAnalysisAgentClient(config)

    # 场景1：基础执行
    print("1. 基础功能验证")
    result = client_basic.execute(
        f"用3句话介绍 SQL 数据分析 Agent 的主要应用场景",
...
```java
// SQL 数据分析 Agent 的 Java 集成示例
@RestController
@RequestMapping("/api/v1/sql-data-analysis-agent")
@Slf4j
public class SqlDataAnalysisAgentController {

    private final SqlDataAnalysisAgentService service;
    ...
    public ResponseEntity<ExecutionResult> execute(
        ...
        log.info("SQL 数据分析 Agent 请求: traceId={}, query={}", traceId, request.query());

        try {
            ExecutionResult result = service.execute(request.query(), request.context());
            return ResponseEntity.ok(result);
        } catch (Exception e) {
            log.error("SQL 数据分析 Agent 执行失败: {}", e.getMessage(), e);
...
// 服务实现
@Service
public class SqlDataAnalysisAgentService {
    ...
    public ExecutionResult execute(String query, Map<String, Object> context) {
        ...
        String response = chatClient.prompt()
            .system("你是专业的 SQL 数据分析 Agent 执行器")
...
- [LangChain SQL 数据分析 Agent 文档](https://python.langchain.com/docs)
