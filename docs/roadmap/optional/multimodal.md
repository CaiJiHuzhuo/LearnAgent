...
处理图像、音频、视频等多种模态数据，让 AI Agent 能理解发票图片、处理语音客服、分析界面截图。
...
class MultimodalClient:
    ...
    def execute(self, task: str, context: Dict[str, Any] = None) -> Dict:
        """执行任务"""
        system_prompt = f"你是专业的 多模态能力 执行器。按照用户要求完成任务。"
        user_msg = task
        if context:
...
executor = MultimodalClient(config)
result = executor.execute("演示 多模态能力 的基本用法")
...
class AdvancedMultimodal:
    ...
    def execute_step(self, step: Dict, previous_results: List) -> TaskResult:
        ...
        response = self.client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "你是 多模态能力 执行专家，按步骤完成任务。"},
    ...
    def execute_plan(self, goal: str) -> TaskResult:
        """执行完整计划"""
        logger.info(f"开始执行 多模态能力 任务: {goal[:50]}")
...
# 演示
agent = AdvancedMultimodal(MultimodalConfig())
result = agent.execute_plan("多模态能力 在支付系统中的应用场景分析")
...
# 构建 多模态能力 流水线
pipeline = ProductionMultimodalPipeline()
...
def stage_process(data: Dict) -> Dict:
    ...
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[{"role": "user", "content": f"处理 多模态能力 请求: {data.get('query')}"}],
        max_tokens=200
    )
...
async def run_pipeline_demo():
    print(f"=== 多模态能力 流水线演示 ===")
    result = await pipeline.run({"query": "支付系统中 多模态能力 的实际应用"})
    print(f"\n最终结果: {result['success']}")
    if result["success"]:
...
async def complete_optional_exercise():
    ...
    # 初始化
    config = MultimodalConfig(debug_mode=True)
    client_basic = MultimodalClient(config)

    # 场景1：基础执行
    print("1. 基础功能验证")
    result = client_basic.execute(
        f"用3句话介绍 多模态能力 的主要应用场景",
...
public class MultimodalController {
    ...
    public ResponseEntity<ExecutionResult> execute(
        ...
        log.info("多模态能力 请求: traceId={}, query={}", traceId, request.query());

        try {
            ExecutionResult result = service.execute(request.query(), request.context());
            return ResponseEntity.ok(result);
        } catch (Exception e) {
            log.error("多模态能力 执行失败: {}", e.getMessage(), e);
...
- [LangChain 多模态能力 文档](https://python.langchain.com/docs)
