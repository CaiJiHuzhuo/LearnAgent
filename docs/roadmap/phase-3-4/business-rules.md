...
将业务规则与 LLM 推理结合，确保 AI 决策符合业务约束和合规要求。
...
# 基础实现示例
def basic_business_rules_example():
    """演示 业务规则引擎 的基础用法"""
    print("业务规则引擎 基础示例")
    # 实际实现见进阶部分

basic_business_rules_example()
```
...
async def demo():
    ...
    service = BusinessRulesService(config)
...
class ProductionBusinessRules:
    ...
    def _do_process(self, input_data: dict) -> dict:
        response = self.client.chat.completions.create(
            ...
            messages=[{
                "role": "user",
                "content": f"处理 业务规则引擎 任务：{input_data}"
            }],
            temperature=0.1
        )
...
# 测试
processor = ProductionBusinessRules()
for i in range(5):
...
public class BusinessRulesService {
    ...
    @Cacheable(value = "business-rules", key = "#request.hashCode()")
    public ProcessResult process(ProcessRequest request) {
        log.info("业务规则引擎 处理请求: {}", request.getQuery());

        String response = chatClient.prompt()
            .system("你是专业的 业务规则引擎 助手")
...
- [LangChain 业务规则引擎 文档](https://python.langchain.com/docs)
