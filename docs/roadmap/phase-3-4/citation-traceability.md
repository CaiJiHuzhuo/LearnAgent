...
引用溯源让 LLM 的输出变得可验证，是金融等高合规场景的必要功能。
...
# 基础实现示例
def basic_citation_traceability_example():
    """演示 引用溯源 的基础用法"""
    print("引用溯源 基础示例")
    # 实际实现见进阶部分

basic_citation_traceability_example()
```
...
class CitationTraceabilityConfig(BaseModel):
    """配置类"""
    enabled: bool = True
    max_results: int = 10
    threshold: float = 0.7
...
class CitationTraceabilityService:
    ...
    def __init__(self, config: CitationTraceabilityConfig):
        self.config = config
        self.client = OpenAI()
...
# 使用示例
async def demo():
    config = CitationTraceabilityConfig()
    service = CitationTraceabilityService(config)
...
class ProductionCitationTraceability:
    """生产级 引用溯源 实现"""
    ...
    def _do_process(self, input_data: dict) -> dict:
        response = self.client.chat.completions.create(
            ...
            messages=[{
                "role": "user",
                "content": f"处理 引用溯源 任务：{input_data}"
            }],
            temperature=0.1
        )
...
# 测试
processor = ProductionCitationTraceability()
for i in range(5):
...
@Service
@Slf4j
public class CitationTraceabilityService {

    private final ChatClient chatClient;

    @Cacheable(value = "citation-traceability", key = "#request.hashCode()")
    public ProcessResult process(ProcessRequest request) {
        log.info("引用溯源 处理请求: {}", request.getQuery());

        String response = chatClient.prompt()
            .system("你是专业的 引用溯源 助手")
...
- [LangChain 引用溯源 文档](https://python.langchain.com/docs)
...
class IntegratedRAGPipeline:
    """集成 citation-traceability 的完整 RAG 流水线"""
