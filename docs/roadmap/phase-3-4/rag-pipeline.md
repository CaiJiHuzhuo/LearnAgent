...
RAG（检索增强生成）是将向量检索与 LLM 生成结合的完整系统，是目前生产级 AI 问答的主流方案。
...
# 基础实现示例
def basic_rag_pipeline_example():
    """演示 RAG 流水线 的基础用法"""
    print("RAG 流水线 基础示例")
    # 实际实现见进阶部分

basic_rag_pipeline_example()
```
...
async def demo():
    ...
    service = RagPipelineService(config)
...
class ProductionRagPipeline:
    ...
    def _do_process(self, input_data: dict) -> dict:
        response = self.client.chat.completions.create(
            ...
            messages=[{
                "role": "user",
                "content": f"处理 RAG 流水线 任务：{input_data}"
            }],
            temperature=0.1
        )
...
# 测试
processor = ProductionRagPipeline()
for i in range(5):
...
public class RagPipelineService {
    ...
    @Cacheable(value = "rag-pipeline", key = "#request.hashCode()")
    public ProcessResult process(ProcessRequest request) {
        log.info("RAG 流水线 处理请求: {}", request.getQuery());

        String response = chatClient.prompt()
            .system("你是专业的 RAG 流水线 助手")
...
- [LangChain RAG 流水线 文档](https://python.langchain.com/docs)
...
class ProductionPipeline:
    ...
    def process(self, input_text: str, additional_context: dict = None) -> dict:
        ...
        messages = [
            {"role": "system", "content": f"你是专业的 RAG 流水线 系统。"},
